...
> Popcorn Hack
 - We just gave an example of a protocol, ASCII, above. What OSI Layer does ASCII fall into? (Presentation)
...
<br>
 - 1.1.1.1.1 no because it does not contain 8 bit numbers. 
 - 23.23.23.23 yes because it has 8 bit numbers seperated into 4 sections by periods. 
 - 134.492.100.0 no because it is contains numbers that are greater than 255.  
 - 255.256.55.255 no because it is contains numbers that are greater than 255.
 - 2.93.255.19 yes becauuse it contains 8 bit numbers and is in the range of 0-255. 
...
- Unicast
- Broadcast
Broadcast because this is when data is sent from one sender to all possible recievers in the network. 
...
FTP and TLS and ASCII are on the same layer (application) because it focuses on user interactions. 
...
The application layer because its responsible for the network services between user applications. 
#### DNS
...
the main domain would be best.com, subdomain is bob.is.the. 

#### HTTP and HTTPS
1. What is a difference between HTTP and HTTPS? The difference is the level of security as HTTPS has a secure communication protocol. 
<br>
...
<br>
 - What protocol should he use, TCP or UDP? Why? He should use UDP because it has lower latency. 
 - What are some cons of this protocol? Give an example of a potential issue. The cons of this protocol is that it is not as reliable and has no error recovery. An example of this issue is that when a packet is lost there is no mechanism to retrive it. 


2. TCP has error checking, which ensures that all packets arrive properly. Why is this important? This is important because ensures reliable and accurate data transmission. 
<br>
 - Give an example of how TCP ensures that there are no errors.
 when downloading a file using TCP, each part of the file will be sent in individual packets and TCP checks for errors. 
...
<br>
 ...
 Server B sends ACK to show that it has successfully recived packet. 
 - What is another use of this?
 In web applications, the integrity of the data is important so the TCP allows for reliable and error checking manners. 
